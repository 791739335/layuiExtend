...
layui.define(["jquery", "laytpl", "layer"], function (exports) {
    ...
    function Cascader(option) {
        ...
        this.changeOnSelect = false; // 是否允许选择任意一级的选项
    ...
    Cascader.prototype = {
        ...
        initOption: function () {
            ...
            self.option.changeOnSelect ? (function () {
                self.changeOnSelect = self.option.changeOnSelect;
            })() : (function () {
                self.changeOnSelect = false;
            })();
        ...
        // 初始化容器和标签
        init: function () {
            $(this.elem).after('<div class="cascader-icon"><i class=" layui-icon layui-icon-down"></i></div>');
        ...
        finishInitData: function () {
            ...
            if (!this.changeOnSelect) {
                ...
                $(this.elem).siblings(".cascader-icon").find('i').replaceWith('<i class="layui-icon layui-icon-down"></i>');
            }
            // 如果有初始值，则第一次不回调
            this.count++;
        ...
        callback: function () {
            ...
            self.domContent.on(self.triggerType, ".urp-cascader-child li", function (event, triggerData) {
                ...
                // 判断当前是否存在子层
                ("children" in self.blockData) ? (function () {
                    if (self.changeOnSelect) self.finishInitData();
                    // 初始化子层
                    self.initChild();
                })() : (function () {
            ...
            $(self.elem).on("click", function () {
                ...
                if (self.onOff) {
                    ...
                    $(self.elem).siblings(".cascader-icon").find('i').replaceWith('<i class="layui-icon layui-icon-up"></i>');
                    self.domContent.css("zIndex", zIndex);
                } else {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideUp(100);
                    $(self.elem).siblings(".cascader-icon").find('i').replaceWith('<i class="layui-icon layui-icon-down"></i>');
                }
            });
            // 图标点击显示隐藏
            $(".cascader-icon").on('click', function () {
                self.onOff = !self.onOff;
                zIndex++;
                if (self.onOff) {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideDown(100);
            ...
            $(document).on("click", function (event) {
                if (event.target.isEqualNode($(self.elem).get(0)) || event.target.isEqualNode($(self.elem).siblings(".cascader-icon").get(0))) return;
                if ($(event.target).hasClass("layui-icon-down")) return;
                self.onOff = false;
                if (!self.onOff) {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideUp(100);
                    $(self.elem).siblings(".cascader-icon").find('i').replaceWith('<i class="layui-icon layui-icon-down"></i>');
